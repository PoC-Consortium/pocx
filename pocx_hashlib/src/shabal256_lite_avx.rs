...
mod tests {
    ...
    #[test]
    fn test_shabal256_lite_avx() {
        if !is_x86_feature_detected!("avx") {
            println!("SKIPPED: test_shabal256_lite_avx - AVX not supported by CPU");
            return;
        }
        ...
        let avx_qualities_c = unsafe { shabal256_lite_128(&simd_data_c, &gensig_c) };

        // Verify all lanes produce the expected result for test C
        for (i, &quality) in avx_qualities_c.iter().enumerate() {
            assert_eq!(
                quality, TEST_C_RESULT,
                "AVX test C: quality[{}] {} doesn't match expected {}",
        ...
        let avx_qualities_d = unsafe { shabal256_lite_128(&simd_data_d, &gensig_d) };

        // Verify all lanes produce the expected result for test D
        for (i, &quality) in avx_qualities_d.iter().enumerate() {
            assert_eq!(
                quality, TEST_D_RESULT,
                "AVX test D: quality[{}] {} doesn't match expected {}",
